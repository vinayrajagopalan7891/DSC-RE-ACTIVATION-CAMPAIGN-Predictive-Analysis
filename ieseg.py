...
def roc(dataSet: DataFrame, actuals: str, probability: str) -> DataFrame:
    ...
    (fpr, tpr, threshold) = roc_curve(
        array(dataSet[actuals]), array(dataSet[probability]), pos_label=1
    )
...
def lift(
    dataSet: DataFrame, actuals: str, probability: str, precision: int = 20
) -> DataFrame:

    summary = cumulativeResponse(
        dataSet=dataSet, actuals=actuals, probability=probability, precision=precision
    )

    summary["Lift"] = (
        summary["Cumulative response"] / Series(summary["Average response"]).max()
    )
    summary["Base"] = (
        summary["Average response"] / Series(summary["Average response"]).max()
    )

    return summary[["Quantile", "Lift", "Base"]]


def cumulativeResponse(
    dataSet: DataFrame, actuals: str, probability: str, precision: int = 20
) -> DataFrame:

    internalSet = equifrequentBinning(
        dataSet=dataSet[[actuals, probability]], byColumn=probability, into=precision
    )
    ...
    summary = (
        internalSet[["Quantile", actuals, "obs"]]
    ...
    summary["Cumulative response"] = (
        summary["cumulativeTarget"] / summary["cumulativeAll"]
    )
    summary["Average response"] = (
        Series(summary["cumulativeTarget"]).max()
        / Series(summary["cumulativeAll"]).max()
    )
...
def cumulativeGains(
    ...
    summary["Cumulative gains"] = (
        summary["cumulativeTarget"] / Series(summary["cumulativeTarget"]).max()
    )
...
def equifrequentBinning(dataSet: DataFrame, byColumn: str, into: int) -> DataFrame:
    ...
    for q in quantile:
        ...
        internalSet.loc[internalSet[byColumn] >= upperBound, byColumn + "_bin"] = int(
            q * into + 1
        )
...
def partition(dataFrame: DataFrame, splitStrategy: [float]) -> [DataFrame]:
    def assignPartition(
        toDataFrame: DataFrame, lowerBound: float, upperBound: float, index: int
    ) -> int:
        if (
            toDataFrame["random"] >= lowerBound * observations
            and toDataFrame["random"] < upperBound * observations
        ):
            return index
        else:
    ...
    for index, split in enumerate(splitStrategy):
        ...
        data["Split"] = data.apply(
            lambda x: assignPartition(x, lowerSplit, upperSplit, index + 1), axis=1
        )
    ...
    for i in range(len(splitStrategy)):
        partitions.append(
            data.loc[data["Split"] == i + 1]
            .drop(["Split", "random"], axis=1)
